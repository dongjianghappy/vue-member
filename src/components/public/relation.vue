<template>
    ...
    <v-dialog v-model:show="isShow" ref="form" title="关联标题" :style="{width: 450, height: 500}" :confirm="true" @submit="submit">
        <template v-slot:content>
            <div>
                <div class="talk-search">
                    <div class="search-box">
                        <i class="iconfont icon-search"></i>
                        <input type="text" placeholder="关键词检索" class="w-full" v-model="content" @input.enter="handleSearch">
                    </div>
                </div>
                <ul style="height: 300px; overflow: auto;">
                    <li v-for="(item, index) in dataList" :key="index" class="ptb10 flex" @click="handleClick(item)">
                        <div class="nowrap" style="flex: 1"><i class="iconfont icon-dot" /> {{item.title || item.name}}</div>
                        <div class="w120">{{item.times}}</div>
...
function submit() {
  emit('choose', currentRelation.value)
  isShow.value = false
}
</script>
