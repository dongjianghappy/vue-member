<template>
<v-popover content="<i class='iconfont icon-topic'></i>话题" arrow="tb" offset="right" :move="-10" keys="popover-topic" @onClick="handleclick">
    ...
    <template v-if="dataList.length">
      <div class="module-wrap">
        ...
        <div class="module-content plr15" style="height: 205px; overflow-y: auto;">
          <div class="col-md-4 pb15" v-for="(item, index) in dataList" :key="index" @click="chooseTopic(`#${item.name}#`)">{{`#${item.name}#`}}</div>
        </div>
      </div>
    </template>
    <v-loding v-else />
