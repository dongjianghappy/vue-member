<template>
<div class="talk-home w1100" style="margin-top: -20px;" :style="`background: linear-gradient(to right, #f5e5c1, rgba(0, 0, 0, 0), rgba(0, 0, 0, 0), #f5e5c1), url(${userInfo.home_background}) no-repeat; background-size: cover`">
  ...
  <div class="home-info relative" style="bottom:20px;">
    <div class="absolute" style="left: -45px;">
      ...
      <p class="ptb10 font12" style="color: #8bc34a;">
        <v-selectbackground kind="home_background" :style="{width: 1170, height: 570}" :img="{width: 1440}" :size="{width: 1000, height: 260}" />
  ...
  <div class="user-info">
    <ul class="align_right">
      <li class="inline font14  current ml10 plr15" v-for="(item, index) in menu" :key="index" @click="handleclick(item.path)">{{item.name}}</li>
    </ul>
  </div>
</div>
</template>
