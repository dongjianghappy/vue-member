<template>
  <div class="module-wrap">
    <div class="module-head">
      ...
      <span class="right"
            @click="handelclick"><i class="iconfont icon-refresh1"></i>点击刷新</span>
    </div>
    <div class="module-content font12">
      <ul>
        <li v-for="(item, index) in hotTalkList[current]"
            :key="index">
          <div class="item-title relative pl15 pr40 ">
            <a class="ptb10 block"
               @click="huati(item.name)">{{item.name}}<span class="mark vote"
                    v-if="item.vote !=='0'">票</span></a><span class="absolute cl-ccc "
                  style="top: 10px ; right: 15px ">{{item.num}}</span>
          </div>
        </li>
      </ul>
    </div>
