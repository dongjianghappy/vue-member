<template>
    <header class="head-fixed">
        <div class="head-wrap">
            <!-- LOGO -->
            <div class="header-left flex" style="width: 190px; height: 60px; line-height: 60px; color: #333;">
              <div class="align_center" style="width: 60px;">
                <Navicon />
              </div>
              <div style="flex: 1"><a :href="`${loginuser.account ? '/u/'+loginuser.account : '/home'}`" target="_brank" class="font24">{{siteInfo.talk_site_name}}</a></div>
            </div>
            <!-- 导航 -->
            <div class="header-content">
                ...
                <ul class="w600 right">
                    ...
                    <li @click="handleSave" v-if="loginuser.account">
                        <i class="iconfont icon-theme1" :class="{'cl-666': !editPendant}" />
                    </li>
                </ul>
            </div>
        </div>
...
import Navicon from '../drawer/navicon.vue'
...
function handleClick(param: any) {
  ...
  if (param === 'signOut') {
    ...
    router.push(param)
  } else if (!loginuser.value.currentUser) {
