    ...
    <li class="col-sm-6 col-md-6 p10"
      ...
      <div class="grid-wrap"
        ...
        <div class="right"
          ...
          <span class="concern"
                @click="concern(item)"
                style=" width:auto; height:20px; line-height:20px; display:block; text-align:center; cursor:pointer; position: absolute; top: 25px; right: 80px;">
            {{item.concern_status}}</span>
          <span class="infos demoimg"
                data-placement="top"
                data-toggle="tooltip17"
                style="width:60px; height:20px; line-height:20px; display:block; text-align:center; cursor:pointer; position:absolute; top:25px; right:15px;"
                data-left="-90">
            <Popover content="<i class='iconfont icon-down font18'></i>"
                     arrow="tb"
                     offset="right"
                     :move="-60"
                     :keys="`popover-${index}`"
              ...
              <div class="p15"
                ...
                <ul class="font14"
                  ...
                  <li style="height: 32px"
                      v-for="(data, i) in group"
                      :key="i"
                      @click="handleClick({ friend_uid: item.friend_uid, grouping: data.id })">{{data.name}}({{data.num}})</li>
                </ul>
              </div>
            </Popover>
          </span>
